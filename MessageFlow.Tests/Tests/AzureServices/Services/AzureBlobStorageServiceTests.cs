...
//        _blobClientMock
//            .Setup(c => c.UploadAsync(It.IsAny<Stream>(), It.IsAny<BlobHttpHeaders>(), It.IsAny<CancellationToken>()))
...
//    {
//        _blobClientMock.Setup(x => x.DeleteIfExistsAsync(null, null, default))
...
//            .ReturnsAsync(Response.FromValue(BlobsModelFactory.BlobDownloadContentResult(content), Mock.Of<Response>()));
