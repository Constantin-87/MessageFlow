...
public class AzureBlobStorageServiceTests
{
    ...
    private readonly Mock<IBlobRagHelper> _ragHelperMock = new();
    ...
    {
        var service = new AzureBlobStorageService(_loggerMock.Object, _blobServiceClientMock.Object, _ragHelperMock.Object);
    ...
    [Fact]
    public async Task DownloadFileContentAsync_ReturnsEmpty_WhenDownloadResponseIsNull()
    {
        _blobClientMock.Setup(x => x.ExistsAsync(default)).ReturnsAsync(Response.FromValue(true, Mock.Of<Response>()));
        _blobClientMock.Setup(x => x.DownloadContentAsync(default)).ReturnsAsync((Response<BlobDownloadResult>)null!);
