...
namespace MessageFlow.Tests.UnitTests.AzureServices.Services;
...
public class AzureSearchServiceTests
{
    ...
    [Fact]
    public async Task UploadDocumentsToIndexAsync_LogsRequestFailedException()
    {
        var dto = new ProcessedPretrainDataDTO
        {
            Id = "x",
        ...
        var blobMock = new Mock<IAzureBlobStorageService>();
        blobMock.Setup(b => b.DownloadFileContentAsync(It.IsAny<string>())).ReturnsAsync("content");
        ...
        var serviceMock = new Mock<AzureSearchService>("https://test", "key", blobMock.Object, logger.Object)
        {
            CallBase = true
        };

        serviceMock.Protected().Setup<SearchClient>("GetSearchClient", ItExpr.IsAny<string>())
            .Returns(searchClient.Object);

        await serviceMock.Object.UploadDocumentsToIndexAsync("company1", new() { dto });

        logger.VerifyLogContains(LogLevel.Error, "Azure Search API Error");
    }

    [Fact]
    public async Task UploadDocumentsToIndexAsync_LogsGeneralException()
    {
        var dto = new ProcessedPretrainDataDTO
        {
            Id = "x",
        ...
        var searchClient = new Mock<SearchClient>();
        searchClient.Setup(s => s.UploadDocumentsAsync(It.IsAny<IEnumerable<Dictionary<string, object>>>(), null, default))
            .ThrowsAsync(new InvalidOperationException("Generic failure"));
        ...
        logger.VerifyLogContains(LogLevel.Error, "General error uploading documents");
    }

    [Fact]
    public async Task UploadDocumentsToIndexAsync_HandlesSuccessfulUploadResults()
    {
        var dto = new ProcessedPretrainDataDTO
        {
            Id = "x",
        ...
        var successResult = SearchModelFactory.IndexingResult("x", null, true, 200);
        var response = SearchModelFactory.IndexDocumentsResult(new[] { successResult });
        ...
        logger.VerifyLogContains(LogLevel.Error, "Failed to index document", Times.Never());
    }
...
{
    // With explicit Times
    public static void VerifyLogContains<T>(this Mock<ILogger<T>> mockLogger, LogLevel level, string message, Times times)
    {
